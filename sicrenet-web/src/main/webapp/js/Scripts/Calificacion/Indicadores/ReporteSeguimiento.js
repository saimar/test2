...
function GetPerfilUsuario() {
    ...
    peticionAjax("ReporteSeguimiento.aspx/GetPerfilUsuario", "POST", null, function (data) {
        perfilUsuario = data.d;
        if (perfilUsuario == "6" || esPerfil17 == "SI") {
            esAnalista = true;
            esGerente = false;
...
function GetFechaCorte() {
    ...
    peticionAjax('ReporteSeguimiento.aspx/ObtenerFechaCorte', "POST", null, function (data) {        
        $("#txtFecha").val(data.d.split("%%&&")[0]);
        fechaCorteOtra = data.d.split("%%&&")[1] != "" ? data.d.split("%%&&")[1] : data.d.split("%%&&")[0];
        ObtenerTabla( $("#txtFecha").val());
    }, null);
}

function buscaFechacorte() {
    Waiting(true, "Espere por favor. Cargando Información...");
    ObtenerTabla($("#txtFecha").val());
}
...
function ObtenerTabla(fechaCorte) {
    ...
    peticionAjax('ReporteSeguimiento.aspx/ConsultaRelMarcados', "POST", { fechaCorte: fechaCorte, nombreCliente: "" }, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
                ...
                document.getElementById("dvMarcados").style.height = (document.documentElement.clientHeight - 200) + "px";
            }
        }
        else
...
function ListaCompletaRelacionadosOrCriterioBusqueda(opcion, fechaCorte, nombreCliente) {
    Waiting(true, "Espere por favor. Cargando Información...");
    SicreNet.Calificacion.Indicadores.ReporteSeguimiento.ListaCompletaRelacionadosOrCriterioBusqueda(opcion, fechaCorte, nombreCliente, function (response) {
        if (response.value.indexOf("Error") == -1) {
            if (response.value != "") {
                ...
                for (var i = 0; i < JSON.length; i++) {
                    ...
                    arreglov.push("<span style='text-align:left;text-decoration: underline;color: rgb(0, 0, 255);cursor:pointer;' lang='" + (JSON[i].DNC_NOMCLIENTE + "%%&&" + fechaCorte) + "' onclick='DetalleClienteRelacionado_link(this);'>" + JSON[i].DNC_NOMCLIENTE + "</span>");
...
function linkDetalleCliente_Click(obj) {
    ...
    AgregarVtnFlotante("divDetalleCliente", "", "DETALLE CLIENTE (" + $(obj).attr("lang") + ")", "", cadena, 150, 850, false, false, "", "", null);
...
function BtnGuardarClasificacionInfo_Click() {
    ...
    function () {
        ...
        if ($("#cmbTipoRelacion").val() != "0")
            GuardarInfoGralClasificacion($("#txtFecha").val(), $("#btnUpdate").attr("lang"), $("#cmbTipoRelacion").val(), (arrayParametrosClasifClient.length > 0 ? arrayParametrosClasifClient[3] : ""), 5, 1);
        else
...
function GuardarInfoGralClasificacion(fechaCorte, nombreCliente, CveRelacionado, NombrePersonarelacion, clascliente, opcionIf) {
    peticionAjax("BuscaCliente.aspx/GuardarInfoGralClasificacion", "POST", { fechaCorte: fechaCorte, nombreCliente: nombreCliente, CveRelacionado: CveRelacionado, NombrePersonarelacion: NombrePersonarelacion, clascliente: clascliente, opcionIf: opcionIf }, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d == "") {
...
function btnCriterioBusqueda_Click() {
    ...
    $(".calendario").datepicker();
    $("#txtFechaBusqueda").val(fechaCorteOtra);
}
...
function btnBuscarFiltro_Click() {
    if ($("#txtFiltroBusqueda").val().trim() != "" && $("#txtFechaBusqueda").val().trim() != "" && $("#txtFechaBusqueda").val().split('/').length == 3) {
        ...
        if ($("#txtFiltroBusqueda").val().toUpperCase() == "TODOS")
            ...
            ListaCompletaRelacionadosOrCriterioBusqueda(4, $("#txtFechaBusqueda").val(), $("#txtFiltroBusqueda").val());
        $("#btnRegresarDeBusqueda").show();
        if (!esGerente || esPerfil17 == "SI")
