...
function CambiaLblTitulo() {
    ...
    document.getElementById('lblTitulo').textContent = 'CANCELACIÓN DE CARTERA ' + $("#ddl_mes option:selected").html().replace(/\s/g, '').toUpperCase() + ' ' + $("#ddl_anio option:selected").html().replace(/\s/g, '').toUpperCase() + '.';
}
function ddl_Mes_Onchange(obj) {
    if ($(obj).attr("id") == "ddl_anio")
        cambiarElemntosCombosMes();
    CambiaLblTitulo();
    estableceEstatus();
}

function cambiarElemntosCombosMes() {
    var numMeses = $("#ddl_anio").val() == fechaActual.split('/')[2] ? fechaActual.split('/')[1] : 12;

    document.getElementById('ddl_mes').options.length = 0;
    for (var i = 1; i <= parseInt(numMeses); i++) {
        var optionM = document.createElement('option');
        optionM.value = (i < 10 ? "0" + i : i);
        optionM.innerHTML = Meses[i - 1];
        document.getElementById('ddl_mes').appendChild(optionM);
    ...
    if ($("#ddl_anio").val() == fechaActual.split('/')[2])
        $("#ddl_mes").val(fechaActual.split('/')[1]);
}
...
var contador = 0; var actividadSelectCorreoGral = "";
function getTBL(etapa, idEtapa) {
    ...
    for (var i = 0; i < Datos.length; i++) {
        ...
        if (Dato.etapa == etapa) {
            ...
            HTML += contador;
            HTML += '</td>';
            HTML += '<td style="text-align:left; width:32%;' + (Dato.idActividadMailGral == "1" ? 'font-weight:bold;text-shadow: 2px 1px 1px rgb(128, 128, 128);font-size: 10px;" title ="Actividad Asignada para el envio de Correo Gral."' : '"') + ' >';
            HTML += Dato.actividad;
            actividadSelectCorreoGral = Dato.idActividadMailGral == "1" ? Dato.actividad : actividadSelectCorreoGral;
            HTML += '</td>';
            ...
            if (editar == true) {
                ...
                HTML += '<div id="btnBorrarActividad' + contador + '" onclick="borrarActividad(\'' + Dato.noActividad + '_' + Dato.actividad + '_' + Dato.idActividadMailGral + '\');" style="cursor: hand; color:#004824; text-decoration: underline;cursor:pointer" alt="Editar" title="Borrar Actividad">Borrar</div>';
...
function borrarActividad(actividad) {
    MostrarMsj("¿Desea borrar la actividad <span style='font-weight:bold'>'" + actividad.split("_")[1] + "'</span>? <br/> " + (actividad.split("_")[2] == "1" ? " Nota:Esta actividad esta definida para el envio del Correo Gral." : ""), "Mensaje", true, true, false, "Si", "No", "", 300, 140,
    function () {
        $("#divVentanaMensajes").dialog("close");
...
function MuestraDetalleM(titulovtn, opcionGuardar, opcionDivVisible, objDato) {
    var cadena = '<div id="divBloqVtnActualizaResponsablesG" style="width:97%;height:83%;background:Gray;filter: alpha(opacity=80); opacity: 0.9;position:absolute;text-align: center;display:none"> </div><div id="dvDetalleFormulariosDescarga" style="width:100%;height:68%;overflow:hidden;text-align:center;float:left;">';
    ...
    cadena += ' <tr><td></td><td style="float: left;"><input type="checkbox" id="chkEnvioMailGral" />Asignar Envio Correo Gral</td></tr><tr><td height="20px"> <input id="hdIdActividad" type="hidden" /></td> </tr> </table></div>';
    cadena += '</div></div>';
    cadena += "<div style='float:right;'><input type='button' onclick='guardarActividad(" + opcionGuardar + ");' class='classButton'  value='Guardar Edición'/>&nbsp&nbsp&nbsp<input type='button' onclick='quitaDetalle();' class='classButton'  value='Cancelar'/></td></tr></table></td></div>";

    $("#dvActualizaResponsablesG").empty();
    AgregarVtnFlotante("dvActualizaResponsablesG", "", titulovtn, "", cadena, 220, 550, false, false, "", "", null);
    ...
    if (opcionGuardar == 0) {
        for (var i = 0; i < Datos.length; i++) {
            ...
            if (dato.noActividad == objDato) {
                ...
                $("#chkEnvioMailGral").attr("checked", dato.idActividadMailGral == "1" ? true : false);
...
function guardarActividad(opcionGuardar) {
    if (validarCamposNecesario(opcionGuardar == 2 ? true : false) == true) {
        if (opcionGuardar == 1) {
            if ($("#chkEnvioMailGral").is(':checked') == false && actividadSelectCorreoGral == $('#txtActividad').val())
                MostrarMsjQuitarUnicaActividadEnvioCorreoGral(opcionGuardar);
            else
                guardarNuevaActividad($("#hdIdActividad").val(), $('#txtActividad').val(), $('#ddlAresRes').val(), $('#txtFechaI').val(), $('#TxtFechaL').val(), $("#chkEnvioMailGral").is(':checked') == false ? 0 : 1);
        }
            ...
            if ($("#chkEnvioMailGral").is(':checked') == false && actividadSelectCorreoGral == $('#txtActividad').val())
                ...
                editarActividad($("#hdIdActividad").val().split("_")[0], $('#txtActividad').val(), $('#ddlAresRes').val(), $('#txtFechaI').val(), $('#TxtFechaL').val(), $("#hdIdActividad").val().split("_")[1], $("#chkEnvioMailGral").is(':checked') == false ? 0 : 1);
        }
        else {
            if ($("#chkEnvioMailGral").is(':checked') == false && actividadSelectCorreoGral == $('#txtActividad').val())
                MostrarMsjQuitarUnicaActividadEnvioCorreoGral(opcionGuardar);
            else
                insertaNuevaEtapa($('#txtEtapa').val(), ($("#hdIdActividad").val() == "1" ? $('#txtEtapa').val() : $('#selectActividad').val()), $('#ddlAresRes').val(), $('#txtFechaI').val(), $('#TxtFechaL').val(), $("#chkEnvioMailGral").is(':checked') == false ? 0 : 1);
        }
    }
}

function MostrarMsjQuitarUnicaActividadEnvioCorreoGral(opcion) {
    WaitingVtn("divBloqVtnActualizaResponsablesG", true, false, "Guardando Información...");
    MostrarMsj("La Actividad '<span style='font-weight:bold;'>" + $('#txtActividad').val() + "</span>', esta asignada para el Envio de Correo Gral ¿Desea Desactivarla? ", "Mensaje", true, true, false, "Si", "No", "", 300, 145,
                function () {
                    actividadSelectCorreoGral = "";
                    $("#divVentanaMensajes").dialog("close");
                    if (opcion == 1)
                        guardarNuevaActividad($("#hdIdActividad").val(), $('#txtActividad').val(), $('#ddlAresRes').val(), $('#txtFechaI').val(), $('#TxtFechaL').val(), $("#chkEnvioMailGral").is(':checked') == false ? 0 : 1);
                    else if (opcion == 0)
...
///////---------------------------------Nueva Actividad
function guardarNuevaActividad(etapa, actividad, idArea, fI, fF, idEnvioMailGral) {
    actividadSelectCorreoGral = "";
    WaitingVtn("divBloqVtnActualizaResponsablesG", true, true, "Guardando Información...");
    document.getElementById("imgVtnLoading").style.marginTop = "8%";
    var parametros = { etapa: etapa, actividad: actividad, idArea: idArea, periodo: periodo, fI: fI, fF: fF, idPais: PaisSelecXUser, idEnvioMailGral: idEnvioMailGral };
...
//-----------------------------Editar Actividad
function editarActividad(etapa, actividad, idArea, fI, fF, idActividad, idEnvioMailGral) {
    actividadSelectCorreoGral = "";
    WaitingVtn("divBloqVtnActualizaResponsablesG", true, true, "Guardando Información...");
    document.getElementById("imgVtnLoading").style.marginTop = "8%";
    var parametros = { etapa: etapa, actividad: actividad, idArea: idArea, periodo: periodo, fI: fI, fF: fF, idActividad: idActividad, idPais: PaisSelecXUser, idEnvioMailGral: idEnvioMailGral };
...
//-----------------------------Nueva Etapa
function insertaNuevaEtapa(etapa, actividad, idarea, fI, fF, idEnvioMailGral) {
    actividadSelectCorreoGral = "";
    WaitingVtn("divBloqVtnActualizaResponsablesG", true, true, "Guardando Información...");
    document.getElementById("imgVtnLoading").style.marginTop = "8%";
    var parametros = { etapa: etapa, actividad: actividad, idArea: idarea, fI: fI, fF: fF, periodo: periodo, idPais: PaisSelecXUser, idEnvioMailGral: idEnvioMailGral };
...
function GuardaResponsablesXEtapa() {
    ...
    for (i = 1; i <= noFilas + 1; i++) {
        if ($("#chkCob_" + i.toString()).is(':checked') == true) {
            Responsables += $("#chkCob_" + i.toString()).parent().parent().children()[0].textContent;
            Responsables += ',';
        }
    }
...
function MuestraDivActualizaResponsables() {
    ...
    cadena += '<div id="dvDetalleEITblActualizaResponsablesGral" style="width:100%;height:100%;overflow: auto;margin-top: 0px;">  ';
    ...
    cadena += '</div></div>';
    cadena += "<table style='width:90%;height:8%;margin-left: 29px;'><tr style='height:1%'></tr><tr><td style='text-align:left'></td><td style='text-align:right'><input type='button' id='btnNuevoResponsable' onclick='MostrarVtnAgregarNuevoResponsable(true);' class='classButton'  value='Agregar Responsable'/></td>";

    $("#dvActualizaResponsablesGral").empty();
    AgregarVtnFlotante("dvActualizaResponsablesGral", "", "ACTUALIZAR RESPONSABLES", "", cadena, 250, 600, false, false, "", "", null);
...
function CargaCatAreas(data) {
    ...
    try {
        ...
        if (data.d == "") {
            $('#btnNuevoResponsable').attr("disabled", true);
            $('#btnNuevoResponsable').attr('class', 'classButtonDis');
            MostrarMsj("Sin Datos.", "Mensaje", false, true, false, "", "Aceptar", "", 250, 120, null, null, null);
            return;
        }
        var arrayJSON = obtenerArregloDeJSON(data.d, false);
...
function MostrarVtnAgregarNuevoResponsable(esNuevoResponsable) {
    WaitingVtn("divBloqVtnActualizaResponsablesGral", true, false, "Cargando Información...");
    var cadena = '<div id="divBloqVtnActualizaResponsablesG" style="width:97%;height:78%;background:Gray;filter: alpha(opacity=80); opacity: 0.9;position:absolute;text-align: center;display:none"> </div><div id="dvDetalleFormulariosDescarga" style="width:100%;height:70%;overflow:hidden;text-align:center;float:left;">';
    cadena += '<div id="dvDetalleEITblActualizaResponsablesG" style="width:100%;height:100%;overflow: auto;margin-top: 0px;">  ';
    cadena += '</div></div>';
    cadena += "<table style='width:90%;height:8%;margin-left: 29px;'><tr style='height:1%'></tr><tr><td style='text-align:left'></td><td style='text-align:right'><input type='button' onclick='" + (esNuevoResponsable ? "GuardarResponsable();" : "ActualizarResponsable();") + "' class='classButton'  value='" + (esNuevoResponsable ? "Agregar" : "Actualizar") + " Responsable'/></td>";

    $("#dvActualizaResponsablesG").empty();
    AgregarVtnFlotante("dvActualizaResponsablesG", "", (esNuevoResponsable ? "AGREGAR" : "EDITAR") + " RESPONSABLE", "", cadena, 190, 600, false, false, "", "", null);
    ...
    $($("#dvDetalleEITblActualizaResponsablesG").find("span")[0]).html((esNuevoResponsable ? "<b>Nuevo" : "<b>Datos") + " Responsable</b>");
    $("#dvActualizaResponsablesG").on("dialogclose", function (event, ui) {
...
function GuardarResponsable() {
    if (ValidarCamposVacios() && ValidarCorreo()) {
        WaitingVtn("divBloqVtnActualizaResponsablesG", true, true, "Guardando Información...");
        document.getElementById("imgVtnLoading").style.marginTop = "8%";
        var parametros = { Opcion: 3, AreaResp: IdAreaR, NombreResp: $($("#dvDetalleEITblActualizaResponsablesG").find("input")[0]).val(), CorreoElec: $($("#dvDetalleEITblActualizaResponsablesG").find("input")[1]).val(), idPais: PaisSelecXUser, idJerarquia: $($("#dvDetalleEITblActualizaResponsablesG").find("select")[0]).val() };
...
function ValidarCamposVacios() {
    ...
    if (!esCorrectoMAil || !esCorrectoNAme) {
        WaitingVtn("divBloqVtnActualizaResponsablesG", true, false, "Guardando Información...");
        MostrarMsj(cadenaMsj + ".", "Mensaje", false, true, false, "", "Aceptar", "", 250, 110, null, function () { $("#divVentanaMensajesEditResponsables").dialog("close"); WaitingVtn("divBloqVtnActualizaResponsablesG", false, false, "Guardando Información..."); }, null, "divVentanaMensajesEditResponsables");
        $("#divVentanaMensajesEditResponsables").on("dialogclose", function (event, ui) {
            WaitingVtn("divBloqVtnActualizaResponsablesG", false, false, "Guardando Información...");
        });
    }
...
function generarTablaDeRegistrosCalend(listaDeJSON) {
    ...
    for (var filas = 0; filas < listaDeJSON.length; filas++) {
        ...
        var json = listaDeJSON[filas];
        for (var element in json) {
            if (element != "idJerarquia") {
                cad += '<td>';
                cad += json[element];
                cad += '</td>';
            }
        }
        cad += " <td style='width:67px'><span style='text-decoration: underline;color:blue;cursor:pointer;' onclick='EditarResponsableArea_Click(\"" + listaDeJSON[filas].Responsable + "\",\"" + listaDeJSON[filas]["Correo Electronico"] + "\",\"" + listaDeJSON[filas].idJerarquia + "\");'>Editar</span> &nbsp&nbsp<span style='text-decoration: underline;color:blue;cursor:pointer;' onclick='EliminarResponsableArea_Click(\"" + listaDeJSON[filas].Responsable + "\",\"" + listaDeJSON[filas].idJerarquia + "\");'>Borrar</span></td>"
...
var idAreaRespAEditar = "";
function EditarResponsableArea_Click(nombreResponsable, correo, idJerarquia) {
    idAreaRespAEditar = $('#slAreasResponsables').val();
    MostrarVtnAgregarNuevoResponsable(false);
    $($("#dvDetalleEITblActualizaResponsablesG").find("input")[0]).attr("disabled", true);
    $($("#dvDetalleEITblActualizaResponsablesG").find("input")[1]).attr("disabled", true);
    $($("#dvDetalleEITblActualizaResponsablesG").find("input")[0]).val(nombreResponsable);
    $($("#dvDetalleEITblActualizaResponsablesG").find("input")[1]).val(correo);
    $($("#dvDetalleEITblActualizaResponsablesG").find("select")[0]).val(idJerarquia);
}
...
function ActualizarResponsable() {
    if (ValidarCamposVacios() && ValidarCorreo()) {
        WaitingVtn("divBloqVtnActualizaResponsablesG", true, true, "Guardando Información...");
        document.getElementById("imgVtnLoading").style.marginTop = "8%";
        var parametros = { idPais: PaisSelecXUser, AreaResp: idAreaRespAEditar, NombreResp: $($("#dvDetalleEITblActualizaResponsablesG").find("input")[0]).val(), idJerarquia: $($("#dvDetalleEITblActualizaResponsablesG").find("select")[0]).val(), Opcion: 1 };
        peticionAjax("CalendarioDeTrabajo.aspx/EditarEliminarResponsable", "POST", parametros, function (data) {
            WaitingVtn("divBloqVtnActualizaResponsablesG", false, false, "Guardando Información...");
            MostrarMsj(data.d.toString(), "Mensaje", false, true, false, "", "Aceptar", "", 250, 110, null, null, null);
            $("#dvActualizaResponsablesG").dialog("close");
            ObtieneCatResponsables();
...
function EliminarResponsableArea_Click(nombreResponsable, idJerarquia) {
    WaitingVtn("divBloqVtnActualizaResponsablesGral", true, false, "Cargando Información...");
    MostrarMsj("¿Esta seguro que desea eliminar el Responsable <span style='font-weight:bold;'>" + nombreResponsable + "</span>? ", "Mensaje", true, true, false, "Si", "No", "", 300, 145,
        function () {
            $("#divVentanaMensajes").dialog("close");
            WaitingVtn("divBloqVtnActualizaResponsablesGral", true, true, "Cargando Información...");
            document.getElementById("imgVtnLoading").style.marginTop = "8%";
            var parametros = { idPais: PaisSelecXUser, AreaResp: $('#slAreasResponsables').val(), NombreResp: nombreResponsable, idJerarquia: idJerarquia, Opcion: 2 };
            peticionAjax("CalendarioDeTrabajo.aspx/EditarEliminarResponsable", "POST", parametros, function (data) {
                MostrarMsj(data.d.toString(), "Mensaje", false, true, false, "", "Aceptar", "", 250, 110, null, null, null);
                ObtieneCatResponsables();
