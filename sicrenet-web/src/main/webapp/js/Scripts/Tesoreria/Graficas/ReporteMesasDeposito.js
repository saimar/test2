...
function funcionLoadMasterPage() {
    ...
    peticionAjax("ReporteMesasDeposito.aspx/DatosFecha", "POST", null, function (data) {
        if (data.d != "") {
...
function btnGenerarReporte_Click() {
    $("#divGridDatos").html("");
    Waiting(true, "Espere por favor. Cargando Información...");
    SicreNet.Tesoreria.Graficas.ReporteMesasDeposito.GeneraReporteMesaCambio(
   $("#txtFecha").val(),
        function (response) {
...
function ExportarAPdf() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('PdfReporteMesaDeposito', '');
    setTimeout(terminarWait, 6000);
}

function ExportarAExcel() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('ExcelReporteMesaDeposito', '');
    setTimeout(terminarWait, 6000);
}