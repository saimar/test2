...
function funcionLoadMasterPage() {
    ...
    peticionAjax("Default.aspx/GetValorArgumentos", "POST", { opcion: 1 }, function (data) {
        if (data.d == "") {
            Tools.eraseCookie("esCargaGrafica");
...
function sltPeriodo_SelectionChange() {
    ...
    Waiting(true, "Espere por favor. Cargando Información...");
    peticionAjax("Default.aspx/GetValorArgumentos", "POST", { opcion: 2 }, function (data) {
        if (data.d.indexOf("Error") == -1) {
            valorArgumentos = data.d;
            if (valorArgumentos != "") {
                $("#sltPeriodo").val(valorArgumentos.split('|')[1])
                $("#sltMesa").val(valorArgumentos.split('|')[4])
            }
...
function sltaño_SelectionChange(agregarItemsComboAño) {
    ...
    Waiting(true, "Espere por favor. Cargando Información...");
    peticionAjax("Default.aspx/DatosFecha", "POST", null, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
                if (valorArgumentos != "")
...
function btnGraficar_Click() {
    ...
    mostrarTablaGrafica();
    // __doPostBack('cargarGrafica', ($("#txtFecha").val() == "" ? valorFecha : $("#txtFecha").val()) + "|" + $("#sltPeriodo").val() + "|" + $("#sltSemanaMes").val() + "|" + $("#sltaño").val() + "|" + $("#sltMesa").val() + "|" + $("#sltMesa :selected").text());
}
...
function mostrarTablaGrafica() {
    ...
    peticionAjax('Default.aspx/getTablaHTML', "POST", parametros, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
                $("#MainContent_spanTitulo").html($("#sltMesa :selected").text());
...
function mostrarGrafica() {
    peticionAjax('Default.aspx/getDatosYSaveGrafica', "POST", null, function (data) {
        if (data.d != "") {
...
function mostrarGraficaNoPoneSiLosDatosDeLaFechaSonCero() {
    peticionAjax('Default.aspx/getDatosYSaveGrafica', "POST", null, function (data) {
        if (data.d != "") {
            var graficaJSON = data.d;
            var JSON = obtenerArregloDeJSON(data.d, false);
            ...
            for (var i = 0; i < JSON.length; i++) {
                var numExisteCero = 0;
                for (var ii = 0; ii < arrayEncabezados.length; ii++) {
                    if (parseInt(JSON[i][arrayEncabezados[ii]]) == 0)
                        numExisteCero++;
                }
                if (numExisteCero == 0 || numExisteCero < arrayEncabezados.length)
                    arrayCategorias.push(JSON[i].Fecha.split(' ')[0]);
            }
            ...
            for (var ii = 0; ii < arrayEncabezados.length; ii++) {
                ...
                for (var i = 0; i < JSON.length; i++) {
                    ...
                    for (var iii = 0; iii < arrayCategorias.length; iii++) {
                        if (JSON[i].Fecha.split(' ')[0] == arrayCategorias[iii]) {
...
function mostrarOcultarRegistros(obj) {
    var indexFila = 0;
    $('#tblDatosConsumoPorMesa >tbody >tr').each(function () {
        if (indexFila > 9) {
            if ($(obj).attr("lang") == "aa")
...
function editarComentarios(comentario, contraparte, fechaReporte, reporteOrigen, idUsr, obj) {
    ...
    peticionAjax('Default.aspx/ObtenerComentario', "POST", parametros, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "")
    ...
    $("#btnGuardarComentario").on("click",
        function () {
            ...
            peticionAjax('Default.aspx/GuardarComentario', "POST", parametrosG, function (data) {
                if (data.d == "")
...
function ExportarAPdf() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('PdfConsumoXMesa', '');
    setTimeout(terminarWait, 4000);
}

function ExportarAExcel() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('ExcelConsumoXMesa', '');
    setTimeout(terminarWait, 4000);
}