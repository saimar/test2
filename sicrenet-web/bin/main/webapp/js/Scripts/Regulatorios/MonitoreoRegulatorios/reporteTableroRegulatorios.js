...
function loadPageTableroReg() {
    ...
    document.getElementById("divMainG").style.marginLeft = '5px';
...
tableroRegulatorio = (function (window, undefined) {
    ...
    return {
        setPais: function (p) { busca(arrayRel.relacion, consts.var2, p, this.relacion); },
        getPais: function () { return this.relacion[0].FIIdPais; },
        setWidth: function () { $(consts.var1).width(sWidth); },
        getFechaActual: function (d) { var fech = "FechaActual" + d; return relacion[0][fech]; },
        getUsuario: function () {
            var dom = document.getElementById('hUsr');
            return dom.value;
        },
...
function DeterminaEstatusMAX(arreglo, segmento) {
    ...
    if (EsBlanco > 0 && Status < 1) Status = 1;
    if (EsVerde > 0 && Status < 3) Status = 3;
    if (EsAzul > 0 && Status < 4) Status = 4;
    if (EsGris > 0 && Status < 5) Status = 5;
    if (EsAmarillo > 0 && Status < 6) Status = 6;
    if (EsNaranja > 0 && Status < 7) Status = 7;
    if (EsRojo > 0 && Status < 8) Status = 8;
    if (EsMorado > 0 && Status < 9) Status = 9;
    if (EsNegro > 0 && Status < 10) Status = 10;
...
function mostrarFichaTecnica(idPais, idSegmento, idReporte, serie, idPeriodicidad, numRecarga)
{
    ...
    idPais2daVtn = idPais;
    peticionAjax('TableroRegulatorios.aspx/GetFichaTecnica', "POST", { idPais: idPais, idReporte: idReporte, idSegmento: idSegmento, idPeriodicidad: idPeriodicidad, fechaCorte: $("#fchOculta").val().split('/')[2] + '-' + $("#fchOculta").val().split('/')[1] + '-' + $("#fchOculta").val().split('/')[0] },
           function (data) {
               if (data.d != null && data.d.indexOf("ERRORCATCH") == -1) {
                   if (data.d != "") {
                       cargoNuevoArchivoAcuse = false;
                       var JSON = obtenerArregloDeJSON(replaceAll(data.d, '\\', '/'), false);
                       var piePagina = '<div id="container" style="width:100%; margin:0px; border:1px solid gray; ">'
                       piePagina += '       <div class="right" style=" float:left; width:160px; margin:0; padding-top: .5em;" >';
                       piePagina += '           <input type="button" value="Agregar Campo" class="classButtonRegulatorios" onclick="btnAgregarCampo_Click();"/>';
                       piePagina += '       </div>';
                       piePagina += '       <div class="left1" style="float:right; width:450px; margin:0; padding-top: .5em;">';
                       piePagina += '          <div class="left2"  style=" float:left; width:150px; margin:0; padding-top: .5em;">';
                       piePagina += '                 <input id="btnEditar" type="button" value="Editar" class="classButtonRegulatorios" onclick="btnEditarGuardarFichatecnica(this);"/>';
                       piePagina += '          </div>';
                       piePagina += '           <div class="center" style="float:right; width:260px; margin:0; padding-top: .5em;">';
                       piePagina += '                <input id="btnCal" type="button" value="Calendario de entregas" class="classButtonRegulatorios" style="padding-left: 10px;padding-right: 10px;" onclick="VentanaCalendario.despliega(this);"/>';
                       piePagina += '           </div>';
                       piePagina += '      </div>';
                       piePagina += '   </div>';

                       popup('popUpDiv', 'FICHA TÉCNICA REPORTE SERIE ' + serie.toUpperCase(), generaTablaVerFichaTecnica(JSON, idReporte, serie), piePagina);
...
function generaTablaVerFichaTecnica(listaJSON, idReporte, serie) {
    ...
    for (var filas = 0; filas < numMitadFilas; filas++) {
        ...
        for (var element in json) {
            if (element == 'CONCEPTO') {
                cad += '<td style="' + colorTd + 'white-space: pre-wrap;" class="estiloConcepto1">';
                cad += json[element];
                cad += '</td>';
            } else if (element == 'DESCRIPCIÓN') {
                cad += '<td style="' + colorTd + 'white-space: pre-wrap;" class="estiloDescripcion1">';
                ...
                if (listaJSON[filas].CONTROL == "txt")
                    ...
                    cad += '<input alt="' + listaJSON[filas].NAMECOL + '" id="btn_' + listaJSON[filas].NAMECOL + '" type="button" class="classButtonRegulatorios" value="Cargar Acuse" style="display:none;height: 100%;font-size: 10px;" onclick="cargarAcuse(\'' + idReporte + '\',\'' + serie + '\',\'' + parseInt(json[element]) + '\')"/></center>';
                }
        ...
        for (var element in json) {
            if (element == 'CONCEPTO') {
                cad += '<td style="' + colorTd + 'white-space: pre-wrap;" class="estiloConcepto2">';
                cad += json[element];
                cad += '</td>';
            } else if (element == 'DESCRIPCIÓN') {
                cad += '<td style="' + colorTd + 'white-space: pre-wrap;" class = "estiloDescripcion2">';
                ...
                if (listaJSON[numMitadFilas + filas].CONTROL == "txt")
                    ...
                    //cad += '<center><img id="imgDownAcuseDis_' + idReporte + '" src="../../Images/Grales/downloadFileDisabled.png" title="No se ha Cargado Acuse"  style="height: 20px;width: 20px;float:left;cursor:pointer;display:' + (parseInt(json[element]) == 0 ? 'inline' : 'none') + '">';
                    //cad += '<center><img id="imgDownAcuseShow_' + idReporte + '" src="../../Images/Grales/downloadFile.png" title="Clic para Descargar Acuse" onclick="descargaArchivoAcuse(\'' + idReporte + '\',\'' + idPais2daVtn + '\');" style="height: 20px;width: 20px;float:left;cursor:pointer;display:' + (parseInt(json[element]) > 0 ? 'inline' : 'none') + '">';
                    cad += '<center><span id="imgDownAcuseDis_' + idReporte + '" onclick="descargaArchivoAcuse(\'' + idReporte + '\',\'' + idPais2daVtn + '\');" style="height: 20px; float:left; cursor:pointer;" title="Clic para Descargar Acuse">' + json[element] + '</span>';
...
function btnAgregarCampo_Click() {
    var cadena =
        '<div id="divBloqVtnAgregarCampo" style="width:95%;height:83%;background:Gray;filter: alpha(opacity=80); opacity: 0.9;position:absolute;text-align: center;display:none"> </div>' +
        '<div style="width:100%;height:54%;overflow:hidden;text-align:center;float:left;">' +
            '<div id="dvDetalleEITbl" style="width:100%;height:100%;overflow: auto;margin-top: 0px;">  ' +
                '<div style="text-align: center;height: 100%;">' +
                    '<table width="100%">' +
                        '<tr style="height:5px"></tr>' +
                        '<tr>' +
                            '<td class="classAgregarCampo">Descripción Campo</td>' +
                            '<td style="text-align: left">' +
                                '<input type="text" id="txtDescripcionCampo" maxlength="90" onkeyup="txtDescCampo_OnKeyUp();" class="classAgregarCampoCaja" />' +
                            '</td>' +
                        '</tr>' +
                        '<tr>' +
                            '<td class="classAgregarCampo">Nombre Campo</td>' +
                            '<td style="text-align: left">' +
                                '<input type="text" id="txtNombreCampo" disabled="disabled"  class="classAgregarCampoCaja" />' +
                            '</td>' +
                        '</tr>' +
                        '<tr style="height:5px"></tr>' +
                    '</table>' +
    ...
    cadena +=
        '<table style="width:90%;height:8%;margin-left: 29px;">' +
            '<tr style="height:1%"></tr>' +
            '<tr>' +
                '<td style="text-align:left"></td>' +
                '<td style="text-align:right">' +
                    '<input type="button" id="btnAgregarCampo" onclick="btnGuardarCampoFichaTecnica_Click();" class="classButtonRegulatorios" value="Agregar"/>' +
                '</td>' +
            '</tr>' +
        '</table>'
    ...
    $('#divTblcontenido').html('');
    $('#divTblcontenido').html(cadena);
    $('#divTblcontenido').css('height', '150px');
    $('#divTblcontenido').css('width', '500px');
    $('#divTblFooter').html('');
    $('#divTblFooter').css('height', 'px');
...
function enviarArchivoAsincronamente(obj, descReporte) {
    ...
    $("#fchOculta").val().split('/')[2] + '-' + $("#fchOculta").val().split('/')[1] + '-' + $("#fchOculta").val().split('/')[0] + '</span>. </br> ¿Desea Continuar? ', "Mensaje SicreNet", true, true, false, "Si", "No", "", 280, 145,
        function BtnSi() {
            $("#divVentanaMensajes").dialog("close");
            ...
            var parametros = {
                ...
                'idPais': idPais2daVtn,
                'fechaCorte': $("#fchOculta").val().split('/')[2] + '-' + $("#fchOculta").val().split('/')[1] + '-' + $("#fchOculta").val().split('/')[0]
            };
...
function descargaArchivoAcuse(idReporte, idPais) {
    ...
    if (e)
        e.cancelBubble = true;
    window.location.assign("Respuesta.aspx?idReporte=" + idReporte + "&&idPais=" + idPais + "&&fechaCorte=" + $("#fchOculta").val().split('/')[2] + '-' + $("#fchOculta").val().split('/')[1] + '-' + $("#fchOculta").val().split('/')[0])
}
...
function resizeTablaReportesInventariosTableroRAZ() {event.cancelBubble
...
var VentanaCalendario = function () {
    var config = { "url": "/SicreNetv2/Regulatorios/MonitoreoRegulatorios/CalendarioAnual.aspx"}
    return {
        "despliega": function () {
            var self = this;
            ...
            cad += '                  <iframe id="iframCal" src="' + self.getHost() + config.url + '?p=' + tableroRegulatorio.getPais() + '&r=' + tableroRegulatorio.getReporte() + '&u=' + tableroRegulatorio.getUsuario();
            cad += '                 " style="width:100%;height:100%; text-align: center; border:none;" ></iframe>';
            cad +='             </center>';
            cad += '       </div>';
            cad += '       <div style="text-align: center; ">';
            cad += '            <input id="btnCal" type="button" value="Grabar" class="classButtonRegulatorios" style="padding-left: 10px;padding-right: 10px;" onclick="VentanaCalendario.grabar(this);"/>';
            cad += '       </div>';

            $('#divTblcontenido').html('');
            $('#divTblcontenido').html(cad);
            $('#divTblFooter').html('');
            $('#divTblcontenido').css('height', '660px');
            $('#divTblFooter').css('height', '0px');
