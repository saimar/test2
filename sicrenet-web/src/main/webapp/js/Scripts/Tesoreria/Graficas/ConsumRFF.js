...
function funcionLoadMasterPage() {
    ...
    peticionAjax("RFF.aspx/GetValorArgumentos", "POST", { opcion: 1 }, function (data) {
        if (data.d == "") {
            Tools.eraseCookie("esCargaGrafica");
...
function sltPeriodo_SelectionChange() {
    ...
    Waiting(true, "Espere por favor. Cargando Información...");
    peticionAjax("RFF.aspx/GetValorArgumentos", "POST", { opcion: 2 }, function (data) {
...
function sltaño_SelectionChange(agregarItemsComboAño) {
    ...
    Waiting(true, "Espere por favor. Cargando Información...");
    peticionAjax("RFF.aspx/DatosFecha", "POST", null, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
                if (valorArgumentos != "")
...
function mostrarTablaGrafica() {
    ...
    peticionAjax('RFF.aspx/getTablaHTML', "POST", parametros, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
...
function mostrarGrafica() {
    peticionAjax('RFF.aspx/getDatosYSaveGrafica', "POST", null, function (data) {
        if (data.d != "") {
...
function mostrarOcultarRegistros(obj) {
    var indexFila = 0;
    $('#tblDatosRFF >tbody >tr').each(function () {
        if (indexFila > 9) {
            if ($(obj).attr("lang") == "aa")
...
function ExportarAPdf() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('PdfConsumoRFF', '');
    setTimeout(terminarWait, 4000);
}

function ExportarAExcel() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('ExcelConsumoRFF', '');
    setTimeout(terminarWait, 4000);
}