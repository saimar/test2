...
function funcionLoadMasterPage() {
    ...
    peticionAjax("ReporteLCTI.aspx/GetValorArgumentos", "POST", { opcion: 1 }, function (data) {
        if (data.d == "") {
            Tools.eraseCookie("esCargaGrafica");
...
function sltPeriodo_SelectionChange() {
    ...
    Waiting(true, "Espere por favor. Cargando Información...");
    peticionAjax("ReporteLCTI.aspx/GetValorArgumentos", "POST", { opcion: 2 }, function (data) {
...
function sltaño_SelectionChange(agregarItemsComboAño) {
    ...
    Waiting(true, "Espere por favor. Cargando Información...");
    peticionAjax("ReporteLCTI.aspx/DatosFecha", "POST", null, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
                if (valorArgumentos != "")
...
function mostrarTablaGrafica() {
    ...
    peticionAjax('ReporteLCTI.aspx/getTablaHTML', "POST", parametros, function (data) {
        if (data.d.indexOf("Error") == -1) {
            if (data.d != "") {
...
function mostrarGrafica() { 
    peticionAjax('ReporteLCTI.aspx/getDatosYSaveGrafica', "POST", null, function (data) {
        if (data.d != "") {
...
function llenaArregloDeGraficaNoPoneCuandoTodosSonCero(JSON, contenedor) {
    var arrayEncabezados = new Array();
    var i = 0;
    for (var encabezados in JSON[0]) {
        if (encabezados != "Fecha")
...
function ExportarAPdf() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('PdfLCTI', '');
    setTimeout(terminarWait, 6000);
}

function ExportarAExcel() {
    Waiting(true, "Espere por favor. Cargando Información...");
    __doPostBack('ExcelLCTI', '');
    setTimeout(terminarWait, 6000);
}